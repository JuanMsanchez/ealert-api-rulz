function validate(rule, defaults)
{
  if(defaults)
    rule = _.defaults(rule, defaults);

  if(!rule.type)
    return next(new Error("No rule type defined"));

  var errors       = [];
  var filteredRule = {};
  var tmplt        = _.assign(BASE_RULE, RULE_TYPE[type.toLowerCase()]);
  _.forEach(tmplt, function(value, key)
  {
    ...
    return {errors: errors, rule: filteredRule});
};
